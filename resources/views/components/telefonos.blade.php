                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @foreach($telefonos as $tel)
                                {{-- Añadimos data-phone para que JS lo encuentre --}}
                                <tr class="text-slate-600 hover:bg-white transition-colors" data-phone="{{ $tel->telefono }}">
                                    <td class="px-4 py-3 font-bold text-slate-900">{{ $tel->telefono }}</td>
                                    <td class="px-4 py-3 uppercase text-[11px] font-semibold">{{ $tel->origen ?? '---' }}</td>
                                    <td class="px-4 py-3">
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center justify-center gap-2">
                                            {{-- BOTÓN WA UNIFICADO: Inicia siempre en gris y cargando --}}
                                            <a href="https://wa.me/51{{ $tel->telefono }}?text=Hola,%20nos%20comunicamos%20de%20KP%20Invest." 
                                               target="_blank" 
                                               class="whatsapp-btn inline-flex h-8 w-8 items-center justify-center rounded-xl transition-all shadow-sm bg-slate-100 text-slate-400 opacity-50 cursor-wait"
                                               title="Verificando WhatsApp...">
                                                
                                                {{-- Texto de carga inicial --}}
                                                <span class="status-dots text-[10px] font-bold">. . .</span>
                                            ...
                                            {{-- Botón SMS --}}
                                            <a href="https://messages.google.com/web/authentication?launch=true&number=51{{ $tel->telefono }}&body=Hola,%20nos%20comunicamos%20de%20KP%20Invest." 
                                            target="_blank"
                                            class="inline-flex h-8 w-8 items-center justify-center rounded-xl bg-blue-100 text-blue-600 hover:bg-blue-600 hover:text-white transition-all shadow-sm"
                                            title="Enviar SMS">
