@props(['telefonos', 'osiptel_verificados' => []])
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @foreach($telefonos as $tel)
                                @php
                                    // 1. Normalizamos el teléfono de la DB (solo dígitos)
                                    $numeroLimpio = preg_replace('/\D/', '', $tel->telefono);
                                    $prefijoActual = substr($numeroLimpio, 0, 5);
                                    
                                    // 2. Convertimos el array de Osiptel a strings para comparación exacta
                                    $verificadosStr = array_map('strval', $osiptel_verificados);
                                    
                                    // 3. Verificamos si existe el prefijo en lo que devolvió el Python
                                    $esTitular = in_array((string)$prefijoActual, $verificadosStr);
                                @endphp
                                <tr class="text-slate-600 hover:bg-white transition-colors" data-phone="{{ $tel->telefono }}">
                                    ...
                                    <td class="px-4 py-3 uppercase text-[11px] font-semibold">
                                        @if($esTitular)
                                            <span class="text-blue-600 font-bold">
                                                {{-- Lógica dinámica de operador según prefijo --}}
                                                @if(str_starts_with($prefijoActual, '930'))
                                                    MOVISTAR (Osiptel)
                                                @else
                                                    ENTEL PERU
                                                @endif
                                    ...
                                    <td class="px-4 py-3">
                                        @if($esTitular)
                                            <span class="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-600 ring-1 ring-inset ring-emerald-600/20 uppercase shadow-sm">
                                        ...
                                        @else
                                            <span class="text-[10px] text-slate-400 font-medium italic lowercase">sin validar</span>
                                        @endif
                                    </td>

                                    <td class="px-4 py-3">
                                        <span class="inline-flex items-center rounded-lg bg-slate-200/50 px-2 py-0.5 text-[10px] font-bold text-slate-600 ring-1 ring-slate-200 uppercase">
                                            {{ $esTitular ? 'POSTPAGO' : ($tel->tipo ?? '---') }}
                                        </span>
                                    </td>

                                    <td class="px-4 py-3 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            {{-- Botón WhatsApp con validación asíncrona --}}
                                            <a href="https://wa.me/51{{ $tel->telefono }}?text=Hola,%20nos%20comunicamos%20de%20KP%20Invest." 
                                               target="_blank" 
                                               class="whatsapp-btn inline-flex h-8 w-8 items-center justify-center rounded-xl bg-slate-100 text-slate-400 opacity-50 cursor-wait shadow-sm transition-all hover:scale-110">
                                                <span class="status-dots text-[10px]">. . .</span>
                                                <svg class="whatsapp-icon h-4 w-4 hidden" fill="currentColor" viewBox="0 0 24 24">
                                            ...
                                            {{-- Botón SMS estándar --}}
                                            <a href="https://messages.google.com/web/authentication?launch=true&number=51{{ $tel->telefono }}" 
                                               target="_blank" 
                                               class="inline-flex h-8 w-8 items-center justify-center rounded-xl bg-blue-100 text-blue-600 shadow-sm transition-all hover:bg-blue-600 hover:text-white hover:scale-110">
