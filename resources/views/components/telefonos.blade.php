                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @foreach($telefonos as $tel)
                                @php
                                    // Comparamos los primeros 5 dígitos del número actual con los verificados
                                    $esTitular = in_array(substr($tel->telefono, 0, 5), $osiptel_verificados);
                                @endphp
                                <tr class="text-slate-600 hover:bg-white transition-colors" data-phone="{{ $tel->telefono }}">
                                    ...
                                    <td class="px-4 py-3 uppercase text-[11px] font-semibold">
                                        @if($esTitular)
                                            {{-- Ajuste dinámico de operador basado en el cruce --}}
                                            <span class="text-blue-600 font-bold">
                                                {{ str_starts_with($tel->telefono, '930') ? 'MOVISTAR' : 'ENTEL PERU' }}
                                            </span>
                                        @else
                                            {{ $tel->origen ?? '---' }}
                                        @endif
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="https://wa.me/51{{ $tel->telefono }}?text=Hola,%20nos%20comunicamos%20de%20KP%20Invest." 
                                               target="_blank" 
                                               class="whatsapp-btn inline-flex h-8 w-8 items-center justify-center rounded-xl bg-slate-100 text-slate-400 opacity-50 cursor-wait">
